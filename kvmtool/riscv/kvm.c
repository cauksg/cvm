...
#define CVM_INITRD_ALIGN	SZ_4K

static void cvm_load_file(struct kvm *kvm, int fd, unsigned long guest_addr,
			  const char *name, ssize_t *file_size)
{
	struct load_file load;
	struct stat sb;
	void *addr;
	...
	load.src_hva = (unsigned long)addr;
	load.des_gpa = guest_addr;
	load.file_size = sb.st_size;

	if (ioctl(kvm->vm_fd, KVM_LOAD_FILE, &load))
		die_perror("KVM_LOAD_FILE");

	if (munmap(addr, sb.st_size))
		die_perror("munmap");

	*file_size = sb.st_size;
	pr_debug("Loaded %s to 0x%lx (%zd bytes)", name, guest_addr, *file_size);
}
				 ...
				 const char *kernel_cmdline)
{
	...
	if (kvm->cfg.cmode) {
		cvm_load_file(kvm, fd_kernel, kvm->arch.kern_guest_start,
			      "kernel", &file_size);
	} else {
	...
	if (fd_initrd != -1) {
		...
		pos = limit - (sb.st_size + (kvm->cfg.cmode ?
					     CVM_INITRD_ALIGN : INITRD_ALIGN));
		guest_addr = ALIGN(host_to_guest_flat(kvm, pos),
				   kvm->cfg.cmode ? CVM_INITRD_ALIGN :
						    INITRD_ALIGN);
		pos = guest_flat_to_host(kvm, guest_addr);
		if (pos < kernel_end)
		...
		if (kvm->cfg.cmode) {
			cvm_load_file(kvm, fd_initrd, initrd_start, "initrd",
				      &file_size);
		} else {
