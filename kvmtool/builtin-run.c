...
static bool kvm_run_init_stopped;
...
static struct kvm *kvm_cmd_run_init(int argc, const char **argv)
{
	...
	if (ret == INIT_LIST_STOP) {
		kvm_run_init_stopped = true;
		return kvm;
	}
	if (ret < 0)
...
static int kvm_cmd_run_cove_io_test_pause(void)
{
	unsigned int pause_secs = cove_io__test_pause_before_run_secs();

	if (!pause_secs)
		return 0;

	pr_info("COVE-IO test: pausing %u seconds before vCPU run",
		pause_secs);
	while (pause_secs)
		pause_secs = sleep(pause_secs);

	if (cove_io__test_skip("vcpu-run")) {
		pr_info("COVE-IO test: skipping vCPU run after pre-run pause");
		return 1;
	}
...
int kvm_cmd_run(int argc, const char **argv, const char *prefix)
{
	...
	if (kvm_run_init_stopped)
		ret = 0;
	else if (kvm_cmd_run_cove_io_test_pause())
		ret = 0;
	else
