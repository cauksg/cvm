							     ...
							     enum irq_type))
{
	...
	if (vmmio->kvm->cfg.cmode)
		_FDT(fdt_property_cell(fdt, "memory-region",
...
struct virtio_mmio *virtio_mmio__from_dev_hdr(struct device_header *dev_hdr)
{
	if (!dev_hdr || dev_hdr->data != (void *)generate_virtio_mmio_fdt_node)
		return NULL;

	return container_of(dev_hdr, struct virtio_mmio, dev_hdr);
}
