...
static int irq__add_msix_route_common(struct kvm *kvm, struct msi_msg *msg,
				      u32 device_id, bool force_devid)
{
	int r;
	struct kvm_irq_routing_entry *entry;
	...
	*entry = (struct kvm_irq_routing_entry) {
	...
	if (force_devid || kvm->msix_needs_devid) {
...
{
	return irq__add_msix_route_common(kvm, msg, device_id, false);
}

int irq__add_msix_route_devid(struct kvm *kvm, struct msi_msg *msg,
			      u32 device_id)
{
	return irq__add_msix_route_common(kvm, msg, device_id, true);
}
