...
#ifndef CVM_VM_TYPE
#define CVM_VM_TYPE KVM_VM_TYPE
#endif
...
int kvm__exit(struct kvm *kvm)
{
	...
	if (kvm->cfg.cmode && kvm->vm_fd >= 0) {
		cove_io__teardown_all(kvm);
	}
...
int kvm__init(struct kvm *kvm)
{
	...
	kvm->vm_fd = ioctl(kvm->sys_fd, KVM_CREATE_VM,
			   kvm->cfg.cmode ? CVM_VM_TYPE : kvm__get_vm_type(kvm));
	if (kvm->vm_fd < 0) {
