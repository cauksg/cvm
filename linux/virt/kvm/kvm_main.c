...
static struct kvm *kvm_create_vm(unsigned long type, const char *fdname)
{
	...
	kvm->mm = current->mm;
	kvm->cmode = false;
	kvm->cvm_root_ready = false;
#ifdef CONFIG_RISCV
	kvm->cmode = type == SBI_EXT_CVM;
#endif
	...
	if (kvm->cmode) {
		...
		sbi_ret = sbi_ecall(SBI_EXT_CVM, SBI_EXT_CVM_CREATE,
				    __pa(cvm_sbi_params), 0, 0, 0, 0, 0);
		if (sbi_ret.error) {
			pr_err("kvm: CVM create failed: %ld\n", sbi_ret.error);
			r = sbi_ret.error;
			kfree(cvm_sbi_params);
			goto out_err_no_cvm;
		}

		sbi_ret = sbi_ecall(SBI_EXT_CVM, SBI_EXT_CVM_ALLOC_ROOT_PT,
				    __pa(cvm_sbi_params), 0, 0, 0, 0, 0);
		kfree(cvm_sbi_params);
		if (sbi_ret.error) {
			pr_err("kvm: CVM root page table allocation failed: %ld\n",
			       sbi_ret.error);
			r = sbi_ret.error;
			goto out_err_no_cvm;
		}
		kvm->cvm_root_ready = true;
	}
#endif
...
#ifdef CONFIG_RISCV
static unsigned long iie_kernel_page_translate(unsigned long addr)
{
	...
	if (!pgd_present(*pgd))
		return 0;
	if (pgd_leaf(*pgd)) {
		offset = (addr & (PGDIR_SIZE - 1)) >> PAGE_SHIFT;
		return __page_val_to_pfn(pgd_val(*pgd)) + offset;
	}
	...
	if (!p4d_present(*p4d))
		return 0;
	if (p4d_leaf(*p4d)) {
		offset = (addr & (P4D_SIZE - 1)) >> PAGE_SHIFT;
		return _p4d_pfn(*p4d) + offset;
	}
	...
	if (pud_leaf(pudp_get(pud))) {
		offset = (addr & (PUD_SIZE - 1)) >> PAGE_SHIFT;
		return pud_pfn(pudp_get(pud)) + offset;
	}
	...
	if (pmd_leaf(pmdp_get(pmd))) {
		offset = (addr & (PMD_SIZE - 1)) >> PAGE_SHIFT;
		return pmd_pfn(pmdp_get(pmd)) + offset;
	}

	pte = pte_offset_kernel(pmd, addr);
	if (!pte || !pte_present(ptep_get(pte)))
		return 0;

	return pte_pfn(ptep_get(pte));
}

static int cvm_init_bitmap(struct cvm_list_params *bitmap)
{
	unsigned long host_page_num, bitmap_bytes, bitmap_pages;
	void *bitmap_addr;

	host_page_num = get_num_physpages();
	bitmap_bytes = DIV_ROUND_UP(host_page_num, BITS_PER_BYTE);
	bitmap_pages = DIV_ROUND_UP(bitmap_bytes, PAGE_SIZE);

	bitmap_addr = alloc_pages_exact(bitmap_pages * PAGE_SIZE, GFP_KERNEL | __GFP_ZERO);
	if (!bitmap_addr)
		return -ENOMEM;

	bitmap->addr = __pa(bitmap_addr);
	bitmap->ele_num = DIV_ROUND_UP(host_page_num, BITS_PER_LONG);
	bitmap->page_num = bitmap_pages;
	return 0;
}

static void cvm_free_bitmap(struct cvm_list_params *bitmap)
{
	if (bitmap && bitmap->addr)
		free_pages_exact(__va(bitmap->addr), bitmap->page_num * PAGE_SIZE);
}

static int cvm_init_ownershiptable(struct cvm_list_params *page_own_table)
{
	unsigned long host_page_num, page_own_table_num, level1, i, page_vaddr;
	unsigned long *page_own_table_addr;

	host_page_num = get_num_physpages();
	page_own_table_num = DIV_ROUND_UP(host_page_num * 8, PAGE_SIZE);
	page_own_table->page_num = page_own_table_num;

	if (page_own_table_num <= 1024) {
		level1 = 0;
		page_own_table_addr = alloc_pages_exact(page_own_table_num * PAGE_SIZE,
							GFP_KERNEL | __GFP_ZERO);
		if (!page_own_table_addr)
			return -ENOMEM;
	} else {
		level1 = DIV_ROUND_UP(page_own_table_num, 1024);
		if (level1 > 512)
			return -ENOMEM;

		page_own_table_addr = (unsigned long *)get_zeroed_page(GFP_KERNEL);
		if (!page_own_table_addr)
			return -ENOMEM;

		for (i = 0; i < level1; i++) {
			page_vaddr = __get_free_pages(GFP_KERNEL | __GFP_ZERO,
						      get_order(1024 * PAGE_SIZE));
			if (!page_vaddr)
				goto err_level1;
			page_own_table_addr[i] = __pa(page_vaddr);
		}
	}

	page_own_table->level = level1;
	page_own_table->addr = __pa(page_own_table_addr);
	page_own_table->ele_num = host_page_num;
	return 0;

err_level1:
	while (i--)
		free_pages((unsigned long)__va(page_own_table_addr[i]),
			   get_order(1024 * PAGE_SIZE));
	free_page((unsigned long)page_own_table_addr);
	return -ENOMEM;
}

static void cvm_free_ownershiptable(struct cvm_list_params *page_own_table)
{
	unsigned long i;
	unsigned long *addr;

	if (!page_own_table || !page_own_table->addr)
		return;

	addr = __va(page_own_table->addr);
	if (!page_own_table->level) {
		free_pages_exact(addr, page_own_table->page_num * PAGE_SIZE);
		return;
	}

	for (i = 0; i < page_own_table->level; i++) {
		if (addr[i])
			free_pages((unsigned long)__va(addr[i]),
				   get_order(1024 * PAGE_SIZE));
	}
	free_page((unsigned long)addr);
}

static int cvm_init_rootpagetable(struct cvm_list_params *root_pt_params,
				  unsigned long *rpt_addr_list)
{
	unsigned long i, root_pt_va;

	for (i = 0; i < (1UL << MAX_CVM_NUM); i++) {
		root_pt_va = __get_free_pages(GFP_KERNEL | __GFP_ZERO, 2);
		if (!root_pt_va)
			goto err;
		rpt_addr_list[i] = __pa(root_pt_va);
	}

	root_pt_params->addr = __pa(rpt_addr_list);
	root_pt_params->ele_num = 1UL << MAX_CVM_NUM;
	return 0;

err:
	while (i--)
		free_pages((unsigned long)__va(rpt_addr_list[i]), 2);
	return -ENOMEM;
}

static void cvm_free_rootpagetable(struct cvm_list_params *root_pt_params)
{
	unsigned long i;
	unsigned long *addr;

	if (!root_pt_params || !root_pt_params->addr)
		return;

	addr = __va(root_pt_params->addr);
	for (i = 0; i < root_pt_params->ele_num; i++) {
		if (addr[i])
			free_pages((unsigned long)__va(addr[i]), 2);
	}
}

static void cvm_free_memorypool(struct cvm_list_params *chunk_infor_list,
				bool free_all_chunks)
{
	unsigned long i;
	unsigned long *chunk_infor_addr;

	if (!chunk_infor_list || !chunk_infor_list->addr)
		return;

	chunk_infor_addr = __va(chunk_infor_list->addr);
	for (i = 0; i < chunk_infor_list->ele_num; i++) {
		struct cvm_mem_chunk_infor *chunk_infor;

		if (!chunk_infor_addr[i])
			continue;

		chunk_infor = __va(chunk_infor_addr[i]);
		if (free_all_chunks && chunk_infor->chunk_vaddr)
		...
		free_page((unsigned long)chunk_infor);
	}
}

static int cvm_init_memorypool(struct cvm_list_params *chunk_infor_list,
			       unsigned long *chunk_infor_addr,
			       unsigned long chunk_num, bool isrefill)
{
	unsigned long i, j;

	chunk_infor_list->ele_num = chunk_num;
	chunk_infor_list->page_num = DIV_ROUND_UP(chunk_num * sizeof(unsigned long), PAGE_SIZE);
	chunk_infor_list->addr = __pa(chunk_infor_addr);

	for (i = 0; i < chunk_num; i++) {
		struct cvm_mem_chunk_infor *chunk_infor;
		unsigned long *chunk_paddr_list;
		void *chunk_vaddr;

		chunk_infor = (struct cvm_mem_chunk_infor *)get_zeroed_page(GFP_KERNEL);
		if (!chunk_infor)
			goto err;
		...
		chunk_paddr_list = (unsigned long *)get_zeroed_page(GFP_KERNEL);
		if (!chunk_paddr_list) {
			vfree(chunk_vaddr);
			free_page((unsigned long)chunk_infor);
			goto err;
		...
		for (j = 0; j < 512; j++) {
			...
			pfn = iie_kernel_page_translate((unsigned long)chunk_vaddr + j * PAGE_SIZE);
			if (!pfn) {
				free_page((unsigned long)chunk_paddr_list);
				vfree(chunk_vaddr);
				free_page((unsigned long)chunk_infor);
				goto err;
			}
			chunk_paddr_list[j] = pfn << PAGE_SHIFT;
		}
...
err:
	chunk_infor_list->ele_num = i;
	cvm_free_memorypool(chunk_infor_list, true);
	return -ENOMEM;
}

int cvm_mem_manege_init(void)
{
	struct cvm_list_params *bitmap = NULL, *page_own_table = NULL;
	struct cvm_list_params *root_pt = NULL, *chunk_infor_list = NULL;
	unsigned long *rpt_addr_list = NULL, *chunk_infor_addr = NULL;
	unsigned long chunk_num;
	struct sbiret ret;
	int r;

	bitmap = kzalloc(sizeof(*bitmap), GFP_KERNEL);
	page_own_table = kzalloc(sizeof(*page_own_table), GFP_KERNEL);
	root_pt = kzalloc(sizeof(*root_pt), GFP_KERNEL);
	chunk_infor_list = kzalloc(sizeof(*chunk_infor_list), GFP_KERNEL);
	rpt_addr_list = (unsigned long *)get_zeroed_page(GFP_KERNEL);
	chunk_infor_addr = (unsigned long *)get_zeroed_page(GFP_KERNEL);
	if (!bitmap || !page_own_table || !root_pt || !chunk_infor_list ||
	    !rpt_addr_list || !chunk_infor_addr) {
		r = -ENOMEM;
		goto out_free;
	}

	r = cvm_init_bitmap(bitmap);
	if (r)
		goto out_free;

	r = cvm_init_ownershiptable(page_own_table);
	if (r)
		goto out_free;

	r = cvm_init_rootpagetable(root_pt, rpt_addr_list);
	if (r)
		goto out_free;

	chunk_num = (PAGE_SIZE << INITIAL_PAGE_NUM) >> CVM_CHUNK_SIZE;
	r = cvm_init_memorypool(chunk_infor_list, chunk_infor_addr, chunk_num, false);
	if (r)
		goto out_free;

	ret = sbi_ecall(SBI_EXT_CVM, SBI_EXT_CVM_INIT_PAGE_LIST,
			__pa(chunk_infor_list), __pa(root_pt), __pa(bitmap),
			__pa(page_own_table), 0, 0);
	r = ret.error;
	if (r)
		goto out_free;

	free_page((unsigned long)chunk_infor_addr);
	free_page((unsigned long)rpt_addr_list);
	kfree(chunk_infor_list);
	kfree(root_pt);
	kfree(page_own_table);
	kfree(bitmap);
	return 0;

out_free:
	cvm_free_memorypool(chunk_infor_list, true);
	cvm_free_rootpagetable(root_pt);
	cvm_free_ownershiptable(page_own_table);
	cvm_free_bitmap(bitmap);
	if (chunk_infor_addr)
		free_page((unsigned long)chunk_infor_addr);
	if (rpt_addr_list)
		free_page((unsigned long)rpt_addr_list);
...
#define CVM_REFILL_LIMIT 64

static atomic_t cvm_refill_count = ATOMIC_INIT(0);

void reset_KVM_memory_pool_refill_count(void)
{
	atomic_set(&cvm_refill_count, 0);
}
...
int refill_KVM_memory_pool(void)
{
	...
	if (atomic_inc_return(&cvm_refill_count) > CVM_REFILL_LIMIT) {
		pr_warn_ratelimited("kvm: CVM memory pool refill limit reached\n");
		return -ENOMEM;
	}

	refill_chunk_list = kzalloc(sizeof(*refill_chunk_list), GFP_KERNEL);
	chunk_infor_addr = (unsigned long *)get_zeroed_page(GFP_KERNEL);
	if (!refill_chunk_list || !chunk_infor_addr) {
		r = -ENOMEM;
		goto out_free;
	}

	refill_chunk_num = (PAGE_SIZE << REFILL_PAGE_NUM) >> CVM_CHUNK_SIZE;
	r = cvm_init_memorypool(refill_chunk_list, chunk_infor_addr,
				refill_chunk_num, true);
	if (r)
		goto out_free;

	ret = sbi_ecall(SBI_EXT_CVM, SBI_EXT_CVM_REFILL_MEMORY_POOL,
			__pa(refill_chunk_list), 0, 0, 0, 0, 0);
	if (ret.error) {
		r = ret.error;
		goto out_free_pool;
	}
	...
	free_page((unsigned long)chunk_infor_addr);
	kfree(refill_chunk_list);
	return 0;

out_free_pool:
	cvm_free_memorypool(refill_chunk_list, true);
out_free:
	if (chunk_infor_addr)
		free_page((unsigned long)chunk_infor_addr);
	kfree(refill_chunk_list);
...
static void kvm_vm_ioctl_load_file_free_pages(unsigned long *src_hpa_array,
					      unsigned long pages_num)
{
	...
	for (i = 0; i < pages_num; i++) {
		if (src_hpa_array[i])
			free_page((unsigned long)__va(src_hpa_array[i]));
	}
}

static int kvm_vm_ioctl_load_file(struct kvm *kvm, void __user *argp)
{
	struct iie_cvm_sbi_params_load *sbi_load_params = NULL;
	unsigned long *src_hpa_array = NULL;
	struct load_file load_file_t;
	unsigned long pages_num, i;
	struct sbiret ret;
	int r = 0;
	...
	if (copy_from_user(&load_file_t, argp, sizeof(load_file_t)))
		return -EFAULT;

	if (!load_file_t.file_size)
		return 0;

	pages_num = DIV_ROUND_UP(load_file_t.file_size, PAGE_SIZE);
	if (pages_num > ULONG_MAX / sizeof(*src_hpa_array))
		return -EOVERFLOW;

	src_hpa_array = alloc_pages_exact(array_size(pages_num, sizeof(*src_hpa_array)),
					  GFP_KERNEL | __GFP_ZERO);
	if (!src_hpa_array)
		return -ENOMEM;
	...
	for (i = 0; i < pages_num; i++) {
		unsigned long va;
		size_t len = min_t(unsigned long, PAGE_SIZE,
				   load_file_t.file_size - i * PAGE_SIZE);
		...
		if (copy_from_user((void *)va,
				   (const void __user *)(load_file_t.src_hva + i * PAGE_SIZE),
				   len)) {
			free_page(va);
			r = -EFAULT;
			goto out_free_pages;
	...
	sbi_load_params = kzalloc(sizeof(*sbi_load_params), GFP_KERNEL);
	if (!sbi_load_params) {
		r = -ENOMEM;
		goto out_free_pages;
	}
	...
	sbi_load_params->src_hpa_array = (unsigned long *)__pa(src_hpa_array);
	sbi_load_params->vmid_ptr = (struct kvm_vmid *)__pa(&kvm->arch.vmid);

	ret = sbi_ecall(SBI_EXT_CVM, SBI_EXT_CVM_LOAD_FILE,
			__pa(sbi_load_params), 0, 0, 0, 0, 0);
	while (ret.error == TEE_NO_MEMORY) {
		r = refill_KVM_memory_pool();
		if (r)
			goto out_free_pages;

		ret = sbi_ecall(SBI_EXT_CVM, SBI_EXT_CVM_RETRY_LOAD,
				__pa(sbi_load_params), 0, 0, 0, 0, 0);
	}
	r = ret.error;

out_free_pages:
	kfree(sbi_load_params);
	kvm_vm_ioctl_load_file_free_pages(src_hpa_array, pages_num);
	if (src_hpa_array)
		free_pages_exact(src_hpa_array, array_size(pages_num, sizeof(*src_hpa_array)));
	return r;
}
#endif
