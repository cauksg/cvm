				  ...
				  struct kvm_cpu_trap *trap)
{
	...
	unsigned long vsstatus = vcpu->kvm->cmode ? csr->vsstatus :
						       ncsr_read(CSR_VSSTATUS);
	...
	if (vcpu->kvm->cmode)
		csr->vsstatus = vsstatus;
	else
		ncsr_write(CSR_VSSTATUS, vsstatus);

	/* Update Guest SCAUSE, STVAL, and SEPC */
	if (vcpu->kvm->cmode) {
		csr->vscause = trap->scause;
		csr->vstval = trap->stval;
		csr->vsepc = trap->sepc;
	} else {
	...
	vcpu->arch.guest_context.sepc = vcpu->kvm->cmode ? csr->vstvec :
							    ncsr_read(CSR_VSTVEC);
