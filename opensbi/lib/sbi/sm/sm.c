...
{
	if (LOCK_DEBUG)
	printm("[IIE SM@%s_%d] %s try lock\n", __func__,
			current_hartid(), str);
	spin_lock(&sm_alloc_enclave_mem_lock);
	if (LOCK_DEBUG)
		printm("[IIE SM@%s_%d] %s get lock\n", __func__,
		       current_hartid(), str);
}
...
void release_big_sm_lock(const char *str)
{
	...
	if (LOCK_DEBUG)
		printm("[IIE SM@%s_%d] %s release lock\n", __func__,
		       current_hartid(), str);
}

void sm_init()
{
	printm("[IIE Monitor] %s invoked\r\n", __func__);
	// platform_init();
	attest_init();
}
...
// 	printm("[IIE Monitor] %s paddr:0x%lx, size:0x%lx\r\n", __func__,
// 	       paddr, size);
...
// {
// 	uintptr_t retval = 0;
// 	printm("[IIE Monitor %d] %s invoked\r\n", current_hartid(), __func__);
// 	print_m_mode = 1;
// 	retval	     = mm_init(paddr, size);
// 	printm("[IIE Monitor %d] %s return:%ld\r\n", current_hartid(), __func__, retval);
// 	return retval;
// }
...
// 	printm("[IIE Monitor] %s return:%ld\r\n", __func__, retval);
...
// 	printm("[IIE Monitor] %s created return value:%ld \r\n", __func__,
// 	       retval);
// 	return retval;
// }
...
// {
// 	uintptr_t retval;
// 	printm("[IIE Monitor] %s invoked, eid:%ld\r\n", __func__, eid);
...
// 		break;
// 	default:
// 		printm_err("[IIE Monitor@%s] wrong ocall_id(%ld)\r\n",
// 			   __func__, ocall_id);
// 		ret = -1UL;
// 		break;
// 	}
